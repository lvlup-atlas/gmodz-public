...
function PMETA:OfferMoney(amount)
    ...
    for i = 1, #DayZItems do net.WriteUInt(self.OfferedItems[i], 14) end
...
local TRADE_INTERACT_RANGE = 80

local function getTradeTarget(ply)
    local trace = util.TraceLine({
        start = ply:GetShootPos(),
        endpos = ply:GetShootPos() + (ply:GetAimVector() * TRADE_INTERACT_RANGE),
        filter = ply
    })
...
local function handleTradeUse(ply, key)
    if key ~= IN_USE or ply.CantUse then return end

    local ply2 = getTradeTarget(ply)
    ...
    if (ply2.TradeTarget == ply) then
        ...
        ply2.TradeWith = ply
        OpenTrade(ply, ply2)
        timer.Simple(1, function() if IsValid(ply) then ply.CantUse = false end end)
    else
        ...
        ply.CantUse = true
        timer.Simple(1, function() if IsValid(ply) then ply.CantUse = false end end)
        timer.Simple(5, function() if IsValid(ply) then ply.TradeTarget = nil end end)
    end
end
hook.Add("KeyPress", "DayZTradeKeyPress", handleTradeUse)
